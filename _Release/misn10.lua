...
function Update()
    ...
    if not start_done then
        ...
        local function FindUnit(odf)
            ...
            for h in ObjectsInRange(500.0, ccarecycle) do
        ...
        local units = {}
        for h in ObjectsInRange(1000.0, ccarecycle) do
        ...
        for _, h in ipairs(units) do
            ...
            if not ccafighter1 and odf == "svfigh" then
                ccafighter1 = h
            elseif not ccafighter2 and odf == "svfigh" then
                ccafighter2 = h
            end

            if not ccatank1 and odf == "svltnk" then
                ccatank1 = h
            elseif not ccatank2 and odf == "svltnk" then
                ccatank2 = h
            end

            if not ccaartil1 and odf == "svartl" then
                ccaartil1 = h
            elseif not ccaartil2 and odf == "svartl" then
                ccaartil2 = h
            elseif not ccaartil3 and odf == "svartl" then
                ccaartil3 = h
            end

            if not ccaturret1 and odf == "svturr" then
                ccaturret1 = h
            elseif not ccaturret2 and odf == "svturr" then
                ccaturret2 = h
            elseif not ccaturret3 and odf == "svturr" then
                ccaturret3 = h
            end
        end
    ...
    if IsAlive(tugger) and (not chase_tug) and sav_secure then
        local attackers = { ccafighter1, ccafighter2, ccatank1, ccatank2, svartil1, svartil2, ccaartil1, ccaartil2,
            ccaartil3 }
    ...
    if IsAlive(ccatug) and got_position then
        if IsAlive(ccatank1) and (not tank1_follow) then
            Follow(ccatank1, ccatug); tank1_follow = true
        end
        if IsAlive(ccatank2) and (not tank2_follow) then
            Follow(ccatank2, ccatug); tank2_follow = true
        end
    ...
    if (artil1_check < GetTime()) then
        ...
        if IsAlive(ccaartil1) and (not artil1_stop) then
            if (not artil1_underway) then
                Goto(ccaartil1, "artil1_path"); artil1_underway = true
            end
            if GetDistance(ccaartil1, post1_geyser) < 20.0 then
                Defend(ccaartil1); artil1_stop = true
            end
        end
        if IsAlive(ccaartil2) and (not artil2_stop) then
            if (not artil2_underway) then
                Goto(ccaartil2, "artil2_path"); artil2_underway = true
            end
            if GetDistance(ccaartil2, post3_geyser) < 20.0 then
                Defend(ccaartil2); artil2_stop = true
            end
        end
        if IsAlive(ccaartil3) and (not artil3_stop) then
            if (not artil3_underway) then
                Goto(ccaartil3, "relic_path1"); artil3_underway = true
            end
            if GetDistance(ccaartil3, geys2) < 50.0 then
                Defend(ccaartil3); artil3_stop = true
            end
        end
    end
    ...
    if IsAlive(ccafighter1) and IsAlive(ccafighter2) and (not plan_a) then
        if GetScrap(2) > DiffUtils.ScaleRes(15) then
            SetAIP("misn10a.aip"); plan_a = true
        end
    end
