...
-- Source-disabled notes: "ch02008.wav" was an unused cue, and two svhraz units
-- at "walker_attack_3" were prototyped as an additional walker attack.
-- A separate #if 0 ambush spawned two "svfigh" at "walker_attack_1" when the
-- walker came within 300 metres, then ordered both to Attack(walker, 1). It is
-- preserved as a discrete optional wave rather than folded into attack 2.
...
function Save()
    return {
        ...
        camera_arrived = camera_arrived,
        camera_complete = camera_complete,
        camera_ready = camera_ready,
        attack3_destroyed = attack3_destroyed,
        attack5_destroyed = attack5_destroyed,
        apc_spawned = apc_spawned,
        apc_damaged = apc_damaged,
        apc_arrived = apc_arrived,
        apc_trigger1 = apc_trigger1,
        apc_trigger2 = apc_trigger2,
        apc_at_hangar = apc_at_hangar,
        walker_attack1 = walker_attack1,
        walker_attack2 = walker_attack2,
        walker_attack3 = walker_attack3,
        won = won,
        lost = lost,
        opening_sound_time = opening_sound_time,
        camera_pause_time = camera_pause_time,
        sound2_time = sound2_time,
        attack5_destroyed_spawn_time = attack5_destroyed_spawn_time,
        attack8_time = attack8_time,
        convoy_time = convoy_time,
        convoy_attack1_time = convoy_attack1_time,
        walker_time = walker_time,
        hangar_attack5_time = hangar_attack5_time,
        hangar_attack6_time = hangar_attack6_time,
        ...
        apc = apc,
        walker = walker,
        convoy_intercept = convoy_intercept,
...
function Load(state)
    ...
    camera_arrived = state.camera_arrived
    camera_complete = state.camera_complete
    camera_ready = state.camera_ready
    attack3_destroyed = state.attack3_destroyed
    attack5_destroyed = state.attack5_destroyed
    apc_spawned = state.apc_spawned
    apc_damaged = state.apc_damaged
    apc_arrived = state.apc_arrived
    apc_trigger1 = state.apc_trigger1
    apc_trigger2 = state.apc_trigger2
    apc_at_hangar = state.apc_at_hangar
    walker_attack1 = state.walker_attack1
    walker_attack2 = state.walker_attack2
    walker_attack3 = state.walker_attack3
    won = state.won
    lost = state.lost
    opening_sound_time = state.opening_sound_time
    camera_pause_time = state.camera_pause_time
    sound2_time = state.sound2_time
    attack5_destroyed_spawn_time = state.attack5_destroyed_spawn_time
    attack8_time = state.attack8_time
    convoy_time = state.convoy_time
    convoy_attack1_time = state.convoy_attack1_time
    walker_time = state.walker_time
    hangar_attack5_time = state.hangar_attack5_time
    hangar_attack6_time = state.hangar_attack6_time
    ...
    attack5 = state.attack5
    apc = state.apc
    walker = state.walker
    convoy_intercept = state.convoy_intercept
...
function Update()
    ...
    if GetTime() > annoy_time then
        annoy_time = GetTime() + 120.0
        local function Sp(odf, n) for i=1, n do local h = BuildObject(odf, 2, "annoy_1"); Hunt(h, 1) end end
    ...
    if GetTime() > attack5_destroyed_spawn_time then
        ...
        local function Sp6(odf, n) for i=1, n do local h = BuildObject(odf, 2, "attack_6"); Goto(h, "attack_6_path", 1) end end
        Sp6("svfigh", 3); Sp6("svtank", 3); Sp6("svwalk", 3)

        local function Sp7(odf, n) for i=1, n do local h = BuildObject(odf, 2, "attack_7"); Goto(h, GetBase(), 1) end end
    ...
    if walker and IsAlive(walker) then
        if not walker_attack2 and GetDistance(walker, "walker_attack_2") < 200.0 then
            walker_attack2 = true
            local function Sp(odf, n) for i=1, n do local h = BuildObject(odf, 2, "walker_attack_2"); Attack(h, walker, 1) end end
            Sp("svfigh", 2); Sp("svltnk", 2); Sp("svtank", 2)
        end

        if not walker_attack3 and GetDistance(walker, "walker_attack_3") < 200.0 then
            walker_attack3 = true
            local function Sp(odf, n) for i=1, n do local h = BuildObject(odf, 2, "walker_attack_3"); Attack(h, walker, 1) end end
    ...
    if GetTime() > attack8_time then
        ...
        local function Sp(odf, n) for i=1, n do local h = BuildObject(odf, 2, "attack_8"); Goto(h, GetBase(), 1) end end
