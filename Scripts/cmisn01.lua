...
-- Source-disabled notes: the tug could carry an "obdata" nuke object, and a
-- "ch01011.wav" failure cue assignment was prototyped but never played.
-- Source event annotation retained: "Relic picked up by Tug".
...
function Save()
    return {
        ...
        objective4_complete = objective4_complete,
        hangar_identified = hangar_identified,
        recycler_deployed = recycler_deployed,
        tug_near_nav = tug_near_nav,
        arials_spawned = arials_spawned,
        focus_on_explosion = focus_on_explosion,
        ...
        armoury_sound_time = armoury_sound_time,
        wave1_time = wave1_time,
        wave2_time = wave2_time,
        wave3_time = wave3_time,
        wave4_time = wave4_time,
        wave5_time = wave5_time,
        arial1_time = arial1_time,
        arial2_time = arial2_time,
        tug_time = tug_time,
...
function Load(state)
    ...
    objective4_complete = state.objective4_complete
    hangar_identified = state.hangar_identified
    recycler_deployed = state.recycler_deployed
    tug_near_nav = state.tug_near_nav
    arials_spawned = state.arials_spawned
    focus_on_explosion = state.focus_on_explosion
    doing_explosion = state.doing_explosion
    sound10_played = state.sound10_played
    decoy_spawned = state.decoy_spawned
    won = state.won
    lost = state.lost
    user = state.user
    ...
    bb_tower = state.bb_tower
    relic = state.relic
    escort1 = state.escort1
    escort2 = state.escort2
    detectors = state.detectors
    nav_end = state.nav_end
    opening_sound_time = state.opening_sound_time
    armoury_sound_time = state.armoury_sound_time
    wave1_time = state.wave1_time
    wave2_time = state.wave2_time
    wave3_time = state.wave3_time
    wave4_time = state.wave4_time
    wave5_time = state.wave5_time
    arial1_time = state.arial1_time
    arial2_time = state.arial2_time
    tug_time = state.tug_time
    ...
    if objective4_complete and not doing_explosion then
        ...
        if t <= 0 then
            ...
            -- Explode
             ...
             -- Native source chooses xpltrsn/xpltrsq by renderer. Redux Lua does
             -- not expose useD3D, so use its primary explosion variant.
