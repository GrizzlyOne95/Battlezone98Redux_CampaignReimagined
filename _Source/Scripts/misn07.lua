...
local rookie_pilot, rookie_flying, rookie_landed
local camera_time, camera1_on, camera2_on, camera3_on
local mag_show_started, mag_show_done
    ...
    if (not first_objective) and (not alarm_on) and (not out_of_car) then
        ...
        if rookie_found and (rookie_remove_time < GetTime()) and (not rookie_removed) then
            if IsAlive(rookie) then
                ...
                -- Initial Kick Up
                SetVelocity(rookie_pilot, { x = 0, y = 50, z = 0 })
        ...
        if rookie_flying and IsAlive(rookie_pilot) then
            ...
            if dist > 40.0 then
                ...
                -- Override Velocity
                SetVelocity(rookie_pilot, { x = dir.x * speed, y = vertical_push, z = dir.z * speed })
            else
        ...
        if rookie_landed and IsAlive(rookie_pilot) then
            ...
            for h in ObjectsInRange(150.0, rookie_pilot) do
                if IsCraft(h) and (GetTeamNum(h) ~= 1) and (GetTeamNum(h) ~= 0) then -- Enemy (2) or Neutral? Enemy usually.
