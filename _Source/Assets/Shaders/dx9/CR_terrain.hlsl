...
void terrain_fragment(
	...
	uniform float detailTiling,        // Scale factor (default 8.0)
	uniform float detailAlbedoScale,   // Contrast of albedo detail (default 2.0)
	uniform float detailRoughnessScale, // Impact on roughness (default 0.0)
	uniform float detailNormalScale,    // Normal map influence (default 0.0)
...
#if defined(DETAILMAP_ENABLED)
	// Sample detail map with adjustable tiling
	float2 detailUV = frac(finalTexCoords * (detailTiling > 0 ? detailTiling : 8.0));
	float4 detailSample = tex2D(detailMap, detailUV);
	
	// Distance fade to prevent moiré patterns
	float detailDistance = saturate(vDepth * 0.025);
	float3 fullbrightDetail = float3(1, 1, 1);
	
	// 1. Albedo Detail (Multiplicative Grey-Neutral)
	float3 detailAlbedo = detailSample.rgb * (detailAlbedoScale > 0 ? detailAlbedoScale : 2.0);
	float3 fadedAlbedo = lerp(detailAlbedo, fullbrightDetail, detailDistance);
	color = lerp(color, color * fadedAlbedo, diffuseTex.a);

	// 2. PBR Modulation (Roughness/Metallic)
	// Assume Detail Green = Roughness offset, Detail Blue = Metallic offset
	// We use 0.5 as neutral point
	float roughnessOffset = (detailSample.g - 0.5) * detailRoughnessScale;
	roughness = saturate(roughness + roughnessOffset * (1.0 - detailDistance) * diffuseTex.a);
	
	// 3. Micro-Normal Detail (Procedural from Detail Map Albedo)
	// This is a "cheap" normal map generated from the detail map brightness
	#if defined(NORMALMAP_ENABLED)
		float h0 = dot(detailSample.rgb, float3(0.3, 0.59, 0.11));
		float hx = dot(tex2D(detailMap, detailUV + float2(0.005, 0)).rgb, float3(0.3, 0.59, 0.11));
		float hy = dot(tex2D(detailMap, detailUV + float2(0, 0.005)).rgb, float3(0.3, 0.59, 0.11));
		float3 detailNormal = normalize(float3(h0 - hx, h0 - hy, 1.0));
		
		// Blend with main normal
		N = normalize(N + mul(detailNormal, tbn) * detailNormalScale * (1.0 - detailDistance) * diffuseTex.a);
	#endif
#endif
