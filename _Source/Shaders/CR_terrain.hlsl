...
float3 blend_normals_rnm(float3 n1, float3 n2)
{
	n1 = normalize(n1);
	n2 = normalize(n2);
...
void terrain_fragment(
	...
	uniform sampler2D detailNormalMap : register(s10),
#else
	uniform sampler2D detailNormalMap : register(s8),
#endif
...
)
{
	...
	const float kDetailNormalStrength = 1.45;
	const float kTerrainDiffuseBoost = 1.12;
	const float kSpotDiffuseBoost = 1.10;
	const float kSpotSpecularBoost = 1.18;
	const float kSpotHotspotRolloff = 0.08;
	...
	const float kToneStrength = 0.45;
	const float kRoughnessBias = 0.18;
	const float kMetalnessBias = -0.18;
	const float kSpecularClamp = 1.20;
	...
	float detailFreq = saturate((length(ddx(detailUv)) + length(ddy(detailUv))) * 18.0);
	float detailBlend = kDetailNormalStrength * saturate(tileBlendStrength + 0.35) * detailNormalStrength;
	detailBlend *= lerp(1.0, 0.42, detailFreq);
	float2 detailXYScaled = detailNormalXY * detailBlend;
	float detailXYLen = length(detailXYScaled);
	detailXYScaled *= min(1.0, 0.65 / max(detailXYLen, 1e-4));
	float detailZ = sqrt(saturate(1.0 - dot(detailXYScaled, detailXYScaled)));
	float3 detailNormalTex = float3(detailXYScaled, detailZ);
	normalTex = blend_normals_rnm(normalTex, detailNormalTex);
#endif
...
#else
	{
		...
		pixelToLight /= d;
			...
			float specPower = max(materialShininess * lerp(0.65, 1.70, glossForSpec), 1.0);
	...
	float seamSoft = saturate(tileBlendStrength * 0.9);
	float2 atlasCell = frac(vTexCoord * 160.0);
	float edgeDist = min(min(atlasCell.x, 1.0 - atlasCell.x), min(atlasCell.y, 1.0 - atlasCell.y));
	float seamEdge = 1.0 - smoothstep(0.04, 0.14, edgeDist);
	float seamDeriv = saturate((length(ddx(vTexCoord * 160.0)) + length(ddy(vTexCoord * 160.0))) * 1.6);
	float3 diffuseLow = tex2Dbias(diffuseMap, float4(vTexCoord, 0.0, 0.5)).xyz;
	float seamBlend = saturate(seamSoft * seamEdge * lerp(0.25, 0.95, seamDeriv));
	float3 seamReducedDiffuse = lerp(diffuseTex.xyz, diffuseLow, seamBlend);
	...
	float terrainLuma = dot(seamReducedDiffuse, float3(0.299, 0.587, 0.114));
	float derivedGloss = saturate(specLuma * 0.18 + emissiveLuma * 0.01 + terrainLuma * 0.02);
	float derivedMetallic = saturate(specLuma * 0.04 - emissiveLuma * 0.04 - terrainLuma * 0.36);
	float glossMapPresence = saturate(glossTex * 4.0);
	float glossBase = lerp(derivedGloss * 0.55, glossTex, glossMapPresence);
	...
	float metallicBase = lerp(derivedMetallic * 0.12, metallicTex, metallicMapPresence);
	...
	float specScale = lerp(0.26, 0.62, gloss);
	float metallicSpecScale = lerp(0.70, 0.90, metallic);
	...
	float3 metalTint = saturate(lerp(seamReducedDiffuse, specularTex, 0.30));
	...
	oColor.xyz *= diffuseEnergy * lerp(1.0, 0.86, metallic) * ao;
	oColor.xyz += seamReducedDiffuse * rimTerm * 0.16;
	float3 iblDiffuse = sceneAmbient.xyz * seamReducedDiffuse * diffuseEnergy * (kIBLDiffuseStrength * ao);
...
#if defined(SPECULAR_ENABLED) || defined(SPECULARMAP_ENABLED)
	oColor.xyz += specularResult.xyz * saturate(specularTex.xyz) * specScale * metallicSpecScale * fresnelColor;
#endif
...
#if defined(DETAILMAP_ENABLED)
	// detail texture
	float3 detailTex = tex2D(detailMap, frac(vTexCoord * 8)).xyz;
	float detailAmt = saturate(0.28 * detailContrastStrength);
	float detailLuma = dot(detailTex, float3(0.333333, 0.333333, 0.333333));
	float detailSigned = (detailLuma - 0.5) * 2.0;
	float detailGain = detailSigned * detailAmt;
	float detailBright = saturate(detailGain);
	float detailDark = saturate(-detailGain);
	float3 detailContrast = float3(1.0, 1.0, 1.0) + detailBright.xxx * 0.055 - detailDark.xxx * 0.010;
	...
	detailColor = clamp(detailColor, float3(0.99, 0.99, 0.99), float3(1.06, 1.06, 1.06));
	...
	oColor.xyz = lerp(oColor.xyz, oColor.xyz * detailColor, detailBlendMask * 0.34);
#endif
