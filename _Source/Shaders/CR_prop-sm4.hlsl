...
)
{
	...
	float4 dTex = diffuseMap.Sample(diffuseSam, uv) * diffuseColor;
	...
	float3 lightAccum = sceneAmbient.xyz;
	float3 specAccum = float3(0, 0, 0);
	float3 specTex = specularMap.Sample(specularSam, uv).xyz;
	float glossTex = specularMap.Sample(specularSam, uv).a;
	float3 emissiveTex = emissiveMap.Sample(emissiveSam, uv).xyz;
	...
	float specPow = max(materialShininess * lerp(0.5, 2.5, glossTex * glossStrength), 1.0);
	...
	[unroll] for (int i = 0; i < MAX_LIGHTS; ++i)
	{
		...
		lightAccum += lightDiffuse[i].xyz * attenuation * NdotL;

		float3 halfVec = normalize(pixelToLight + viewDir);
		float specNdotH = saturate(dot(viewNormal, halfVec));
		specAccum += lightSpecular[i].xyz * attenuation * pow(specNdotH, specPow);
	}
