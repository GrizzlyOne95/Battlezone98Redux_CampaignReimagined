...
// DX11 Enhanced PSSM v2. Keep this block synchronized with CR_base-sm4.hlsl.
...
static const float CR_TERRAIN_IBL_REFERENCE_FOG_RANGE = 160.0;
static const float CR_TERRAIN_IBL_AIRLESS_FLOOR = 0.15;
...
float terrain_ibl_diffuse_strength(float4 fogParams)
{
    ...
    float atmosphereSupport = saturate(abs(fogParams.w) * CR_TERRAIN_IBL_REFERENCE_FOG_RANGE) * validFogRange;
    return lerp(CR_TERRAIN_IBL_AIRLESS_FLOOR, 1.0, atmosphereSupport);
}
...
)
{
    ...
    lightResult.xyz += irradiance * diffuseEnergy * CR_IBL_DIFFUSE_INTENSITY * terrainDiffuseIblStrength;
