...
- `Open-ModDbHandoff.cmd` — validates the ZIP, copies the submission text, selects the archive in Explorer, and opens the Mod DB upload page.
...
2. Double-click `Open-ModDbHandoff.cmd`. It verifies the ZIP against `moddb_submission.json`, copies the ready-to-paste submission text to the clipboard, selects the ZIP in Explorer, and opens the upload page in Opera. If Opera is unavailable it uses the default browser. The page it opens is the `ModDbUploadUrl` recorded in `moddb_submission.json`; the script refuses any URL that is not an `https` address on `www.moddb.com`.
...
For a command-line integrity check that opens no windows and does not touch the clipboard:
