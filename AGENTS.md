...
- Use `Manage-CampaignFiles.ps1` for deploy/sync, Workshop staging, and publishing. Publishing is a **local** run of that script. The retired `Publish Steam Workshop` GitHub workflow was never a qualified publication path and its dedicated self-hosted runner was never provisioned. Drive the manager non-interactively (`-workshop-init`, `-workshop-auth`, `-workshop-build` for a dry run, `-publish "<note>"`); the bare script opens a `Read-Host` menu that cannot be used headlessly. `workshop.config.json` is per-machine and gitignored, so a fresh clone has none. A new file in the repo is excluded from staging until `-bless` adds it to `Shipping/shipping.lock.json`, which surfaces confusingly as "missing required file". `-bless` refuses to run if the rebuild would **remove** rows, because it rebuilds membership by scanning disk and a generated or unbuilt tree that this machine happens to lack would otherwise be dropped silently; pass `-bless -allow-removals` when files really should stop shipping. `-deploy` verifies the install against the lock when it finishes and exits non-zero if they disagree, so a deploy that skipped shipped files is loud rather than reporting success; `-no-verify` skips that. The live-install actions (`-deploy`, `-fromsource`, `-release`, and the bare menu) no longer self-elevate unconditionally: they write-probe the resolved destination first and raise UAC only if it fails. Both storefronts grant their game folders to ordinary users, so on a normal machine there is no prompt and the deploy runs in the calling console, where its output, its exit code and its verify verdict are visible -- an elevated relaunch is a separate window, which is why a cancelled prompt used to be indistinguishable from a clean run. The probe fails safe, so an install that genuinely needs an administrator still gets one. Publishing also needs a tagged OpenShim release, not a branch. Full sequence in `docs/STEAM_PUBLISH_CHECKLIST.md`. `Local\Workshop` is generated staging, not a runtime. `BZR_CAMPAIGN_RUNTIME_DIR` may override the GOG runtime intentionally but must not point at a Steam Workshop cache.
