...
function Is-ExcludedSourceRelativePath($relativePath) {
    ...
    # The organized folders are the canonical source tree. Root-level runtime
    # files are ignored by Git and may be stale leftovers from old flat deploys,
    # so only the four intentional game deployment descriptors may ship from the
    # root. In particular, native binaries and managed payloads must come from
    # Bin and InstallerPayload rather than an arbitrary ignored root copy.
    if (-not $normalized.Contains("\") -and $SourceDeployableRootFiles -notcontains $normalized) {
        return $true
    }
